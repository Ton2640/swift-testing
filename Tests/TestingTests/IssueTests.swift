...
struct IssueCodingTests {
  ...
  private static let issueKinds: [Issue.Kind] = [
    ...
    Issue.Kind.expectationFailed(Expectation(evaluatedExpression: .__fromSyntaxNode("abc"), isPassing: true, isRequired: true, sourceLocation: #here)),
  ...
  func testCodable(issueKind: Issue.Kind) async throws {
    let issue = Issue(
      kind: issueKind,
      comments: ["Comment"],
      sourceContext: SourceContext(backtrace: Backtrace.current(), sourceLocation: #here)
    )
